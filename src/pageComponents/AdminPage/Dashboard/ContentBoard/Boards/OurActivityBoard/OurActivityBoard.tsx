...
export function OurActivityBoard() {
	...
	const registers = {
		image: register('image', {
			required: ourActivityBoardData?.image ? false : true,
		}),
	};
	...
	useEffect(() => {
		if (ourActivityBoardData) {
			setValue('image', ourActivityBoardData.image);
			clearErrors();
	...
	useEffect(() => {
		watch((value) => {
			if (value.image !== ourActivityBoardData?.image) {
	...
	const onSubmit: SubmitHandler<OurActivityFormFieldValues> = async (
		...
		try {
			if (data.image && data.image.length > 0 && typeof data.image !== 'string') {
				...
				image = ourActivityBoardData?.image || '';
			}
		} catch (error) {
